...
### 🌐 API 設計原則 (2025-06-11 新增)
- **共用優先**：絕對不能遇到新需求就開新 API，必須優先考慮擴充現有 API
- **去除重複**：相似功能必須整合到同一個 API 端點
- **單一職責**：每個 API 只負責一個明確的業務領域
- **擴展而非新增**：透過查詢參數、請求體擴展功能，而非創建新端點

#### API 設計檢查清單
1. **現有 API 能否滿足需求？** - 先查看所有現有端點
2. **能否通過參數擴展？** - 優先使用查詢參數或請求體欄位
3. **是否真的需要新端點？** - 只有在業務領域完全不同時才創建
4. **是否造成功能重複？** - 避免多個 API 做類似的事情
...
GET /api/sabbath-files
GET /api/recent-files

✅ 正確：使用參數擴展單一 API
GET /api/files?category=shared
GET /api/files?category=sabbath
GET /api/files?sort=recent&limit=10
```
...
- **API 精簡原則**：不能遇到什麼就開什麼 API，必須共用和去除重複
...
### 🔐 Cloudflare Access 認證問題處理 (2025-06-12 新增)

#### ⚠️ 關鍵原則：不可跳過 Cloudflare Access 驗證
**生產品質保證要求**：當 MCP 瀏覽器訪問 https://files.94work.net 出現認證要求時，必須完成完整的認證流程，這是真實生產環境的安全機制。

#### 🔍 問題識別標準
當遇到以下情況時，表示 Cloudflare Access 認證問題：
- 瀏覽器重定向到 `/cloudflare-auth` 頁面
- 顯示「需要 Cloudflare 認證」訊息
- 系統要求重新驗證但無法通過

#### 🛠️ 標準排查流程
1. **API 端點檢查**：
   - 檢查 `/api/auth/status` 是否正常回應
   - 驗證後端認證中間件配置
   - 確認開發模式設定是否正確
...
2. **容器服務狀態**：
   ```bash
   podman ps --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}"
   podman logs memoryark2_backend_1
   podman logs memoryark2_nginx_1
   ```

3. **開發環境配置檢查**：
   - 確認 `DEV_BYPASS_AUTH=true` 設定
   - 檢查 `DEV_AUTO_LOGIN_EMAIL` 配置
   - 驗證 JWT 密鑰配置

4. **網路連接驗證**：
   - 直接訪問 `http://localhost:7001` 測試
   - 檢查 nginx 反向代理配置
   - 確認 SSL 憑證狀態

#### 🚫 嚴禁操作
- **不得跳過認證流程**：認證是生產安全的核心
- **不得停用 Cloudflare Access**：這會破壞安全架構
- **不得使用未認證的訪問方式**：影響測試真實性

#### ✅ 正確處理方式
1. **完整調查問題根因**：找出為何無法通過 cloudflare-auth
2. **修復 API 問題**：解決底層認證機制問題
3. **驗證修復結果**：確保認證流程完全恢復
4. **記錄問題與解決方案**：更新故障排除文檔
